...
BRANCH="${CIRCLE_BRANCH/\//-}" #slashes are not valid in docker tags. replace with dashes
...
if [[ "${CIRCLE_BRANCH}" == "production" ]]; then
	...
	DOCKER_BRANCH_IMAGE_VERSION="${BRANCH/\//-}" #slashes are not valid in docker tags. replace with dashes
fi
